...
## Extension: Cosmological Synthesis and Computational Framework

This section describes the theoretical and computational infrastructure underlying the QGP analysis, with particular focus on the multi-tier architecture enabling seamless integration of QGP microphysics, cosmological scales, and GPU acceleration.

### Tier 1: QGP Microphysics Core

The QGP analysis presented above relies on a foundational framework implemented in `src/qgp/`:

- **Constants** (`constants.py`): Single source of truth for all physics parameters (Stromgren sphere, recombination rates, nuclear geometry). All references use named constants, preventing silent numerical errors.
- **Physics Models** (`physics.py`): Woods-Saxon nuclear densities, Glauber Monte Carlo, BDMPS-Z energy loss, hydrodynamic flow, strangeness suppression. ~1000 LOC, fully typed and tested.
- **I/O and Provenance** (`io_utils.py`): Standardized file I/O with provenance headers classifying each data file (MEASURED, PREDICTED, SCHEMATIC).
- **Data Generation** (`generate.py`): Multi-stage orchestrator connecting physics models to output data files, with automatic checksum validation.

**Test Coverage**: 76 tests (qgp/) validating physics constraints, dimensional correctness, and monotonicity properties.

### Tier 2: Cosmological Extension

To contextualize QGP formation within cosmic evolution, we have extended the framework to cosmological timescales via `src/cosmology/`:

#### Dark Energy and Distance Measures
- **Module**: `dark_energy.py` (284 LOC)
- **Physics**: ΛCDM model with parameterized dark energy equation of state ($w = p/\rho$), comoving distances, luminosity distances, distance moduli.
- **Data**: Baryon Acoustic Oscillation (BAO) measurements at $z = 0.5, 0.75, 1.0$ (DESI 2025), sound horizon $r_s = 149.3$ Mpc (Planck 2018).
- **Tests**: 25 tests validating Hubble parameter monotonicity, distance measure scaling, BAO consistency.

#### Reionization Bubble Dynamics
- **Module**: `reionization_bubble.py` (234 LOC)
- **Physics**: Ionization bubble growth, percolation transition from neutral to ionized universe, analogy to QGP bubble nucleation.
- **Model**: Growth timescale $\tau(z) \propto (1-x_e)^{-2}$; overlap probability with percolation threshold at $x_e \sim 0.35$.
- **Tests**: 20 tests validating redshift dependence, critical size transitions, smooth percolation behavior.

#### Reionization Front Propagation (JWST Era)
- **Module**: `reionization_fronts.py` (287 LOC)
- **Physics**: Stromgren sphere equilibrium radius, ionization front expansion into neutral hydrogen, Ly-alpha absorption profile.
- **Key Features**:
  - Stromgren radius: $R_s = (3L / 4\pi \alpha n_H^2)^{1/3}$ for ionizing luminosity $L$ and recombination coefficient $\alpha(T)$.
  - Front expansion: $R(t) = \sqrt{R_s^2 + (early\_time\_growth)^2}$ ensuring smooth transition from early ballistic growth to Stromgren equilibrium.
  - Ly-alpha profile: Lorentzian optical depth $\tau(v) = \tau_0 / (1 + (v/v_D)^2)$ with $\tau_0$ scaling as $(1-x_e)^{2.5}$ to capture ionization dependence.
- **Tests**: 22 tests covering Stromgren sphere properties, monotonic radius growth, transmission profiles, symmetry.

**Design Philosophy**: Tier 2 is fully decoupled from Tier 1. No imports between QGP and cosmology modules. Both tiers bridge at the build system level (shared Makefile, shared pytest), enabling independent publication and extension.

### Tier 3: GPU Acceleration

For numerical-intensive operations (Glauber Monte Carlo, grid computations), Tier 3 provides transparent GPU acceleration via `src/gpu/`:

- **Module**: `gpu/__init__.py` (180 LOC)
- **Abstraction**:
  ```python
  ...
  ```
- **Key Functions**:
  - `pairwise_distance_gpu()`: Vectorized distance matrix for nucleon interactions
  - `energy_loss_grid_gpu()`: Batch BDMPS-Z energy loss on array grids
  - `eccentricity_grid_gpu()`: Vectorized eccentricity tensor computation
- **Fallback**: Zero overhead if CUDA/CuPy unavailable (seamless NumPy fallback)
- **Tests**: 23 tests validating GPU outputs match NumPy exactly, shape preservation, physical constraints.

**Integration Strategy**: Tier 1 calls Tier 3 functions when GPU_AVAILABLE. No mandatory GPU dependency. CI tests both GPU-enabled and CPU-only paths.

### Computational Stack Summary

| Tier | Purpose | LOC | Tests | Dependencies |
|------|---------|-----|-------|--------------|
| 1 | QGP microphysics | ~4000 | 76 | numpy, scipy, matplotlib |
| 2 | Cosmology extension | ~800 | 66 | numpy, scipy |
| 3 | GPU acceleration | ~180 | 23 | numpy, cupy (opt.) |
| **Total** | | **~5000** | **257** | **Minimal external** |

### Build System and Workflow

The centralized Makefile orchestrates all tiers:

```bash
make data          # Generate Tier 1 data files
make figures       # Compile Tier 1 TikZ figures
make test          # Run all 257 tests (Tiers 1–3)
make lint          # Static analysis: ruff + mypy (type-checked)
make strict        # Fail on any warnings (CI gate)
```

**Reproducibility**: All outputs deterministic. RNG seeded with `np.random.default_rng(42)`. Checksums stored in `build/checksums.txt` and validated by `make verify-checksums`.

### Future Extensibility

The architecture is designed for future tiers without modifying existing code:

- **Tier 4 (Machine Learning)**: Independent package importing only `data/*.dat` from Tier 1. No source code coupling.
- **Tier 2b (Cosmology GPU)**: Could add `cosmology/dark_energy_gpu.py` for GPU-accelerated BAO fitting without affecting Tier 1.
- **Tier 1c (Precision QCD)**: Additional physics modules (jet substructure, color glass condensate) could extend Tier 1 independently.

### Summary

The three-tier architecture enables this work to bridge gap between high-energy nuclear physics (QGP) and cosmology, while maintaining clean separation of concerns. Each tier has:
- Independent tests (no coupling via shared fixtures)
- Minimal external dependencies
- Clear data ownership (Tier 1 generates, Tier 2 computes, Tier 3 accelerates)
- Future-proof design for optional GPU acceleration and additional domains

This structure reflects our design philosophy: **build in layers, test in isolation, integrate at the Makefile level**.
